...
| 1 | **CLI version check** — `tap --version` | npx install path (ubuntu + macOS) via `release-smoke.yml`; brew install path no longer smoke-tested in CI | `tap --version` equals the latest `v*` tag on the **tap-core** repo (i.e. `core/npm/cli/package.json` `version` field) — NOT the extension manifest, which is on an independent track. CLI and extension co-release but can be bumped separately. If mismatch, step 0 didn't refresh; fix that first |
